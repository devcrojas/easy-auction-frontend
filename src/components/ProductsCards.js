...
import { Card, Button, TextField, CardContent, CardMedia, Typography, CardActionArea, ListItem, CardHeader, Avatar } from '@mui/material'
...
function ProductsCards() {
    ...
    const cards = () => {
        let card = apis.map((producto) => {
            ...
            let imag = (producto.file) ? producto.file.filePath : '\\sin.jpg'
            return (
                    ...
                    <Card sx={{ width: '100%', height: '100%', borderRadius: 5, marginBottom: 10 }} elevation={10} key={producto._id}>
                        <CardActionArea onClick={() => {
                            ...
                            <CardContent>
                                ...
                                <Row className='my-5'>
                                    <Col>
                                        <ListItem>
                                            <Typography component="div" color='success'>Ofertado:<em><b>$</b></em></Typography>
                                            <Typography component="div" className="text-success">{producto.price.offered} </Typography>
                                        </ListItem>
                                    </Col>
                                </Row>
                            </CardContent>
                        ...
                        <Row>
                            <Col className="text-center" md={7}>
                                <TextField key={producto._id} size="small" label="Ofertar" type="number"  InputProps={{ inputProps: { min: 0  }}}  sx={{ borderRadius:15 }}/>
                                <IconButton aria-label="send">
                                    <SendIcon color="primary" />
                                </IconButton>
                            </Col>
                            <Col className="text-center" md={5}>
                                <Button variant="contained" size="large" color="success" >Comprar ahora</Button>
                            </Col>
                        </Row>
                    </Card>
                </Col>
                ...
                <Modal.Body>
                    <Row>
                        ...
                        <Col md={6} className="fuente">
                            <p><CategoryIcon color="primary" /> Categoria: {detalle.category}</p>
                            <p><CarpenterIcon color="primary" /> Material: {detalle.description.material}</p>
                            <p><AssignmentTurnedInIcon color="primary" /> Marca: {detalle.description.marca}</p>
                            <p><SquareFootIcon color="primary" /> Dimensiones: {detalle.description.dimensions}</p>
                            <p><NewReleasesIcon color="primary" /> Condicion: {detalle.description.actualCondition}</p>
                            <p><RemoveRedEyeIcon color="primary" /> Observaciones: {detalle.description.observations}</p>
                            <p><PaidIcon color="primary" /> Precio inicial:<em><b>$</b></em>{detalle.price.initialP}</p>
                            <p><PaidIcon color="primary" /> Comprar ahora:<em><b>$</b></em>{detalle.price.buyNow}</p>
                            <p><PaidIcon color="primary" /> Precio ofertado:<em><b>$</b></em>{detalle.price.offered}</p>
                ...
                <Zoom scale={0.4}>
                    <img style={{ width: "100%", height: 350, border: 5 }} src={`\\${principal.filePath}`} alt={principal.nameProduct} />
                    {
                        detImages.map(((im, index) => {
                            return <img key={index} style={{ width: "100%", height: 350, border: 5 }} src={`\\${im.filePath}`} alt={principal.nameProduct} />
                        }))
                    }
                </Zoom>
            );
